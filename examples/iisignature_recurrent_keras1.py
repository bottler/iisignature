...
class RecurrentSig(keras.layers.recurrent.Recurrent):
    ...
    def build(self, input_shape):
        ...
        self.W = self.init(
            (self.input_dim, self.n_units), name="{}_W".format(self.name)
        )
        saved_data_length = self.n_units * (
            1 + self.sigsize if self.use_signatures else 1
        )
        self.U = self.inner_init(
            (saved_data_length, self.n_units), name="{}_U".format(self.name)
        )
        self.b = K.zeros((self.n_units,), name="{}_b".format(self.name))

        self.trainable_weights = [self.W, self.U, self.b]
        if self.train_time_lapse:
            log_timelapse = K.variable(
                math.log(self.initial_time_lapse),
    ...
    def step(self, x, states):
        ...
        prev_states_as_memory = (
            K.concatenate([prev_sigs, prev_states], 1)
    ...
    def get_initial_states(self, x):
        ...
        initial_states = [
            K.tile(initial_state, [1, i]) for i in lengths
        ]  # (samples, i)
