...
using SignatureType = std::conditional_t<arbprec, ExactSignature::Signature,
                                         CalcSignature::Signature>;

// the stroke ending at from ... the stroke ending before to
template <bool arbprec>
SignatureType<arbprec> sigPiece(const std::vector<PathReal> &path, int d, int m,
...
template <bool arbprec>
std::vector<double> sig(const std::vector<PathReal> &path, int d, int m,
                        bool log) {
...
template <bool arbprec>
std::vector<double> sigParallel(const std::vector<PathReal> &path, int d, int m,
                                bool log) {
  size_t pathLength = path.size() / d;
  size_t nThreads = 4;
  ...
  for (size_t thread = 0; thread < nThreads; ++thread) {
    ...
    size_t end =
        (thread + 1 == nThreads) ? pathLength : (thread * each + each + 1);
