...
using std::ostream;
template <class S> void checkSize(size_t size, const S &a) {
  if (a.size() != size)
...
template <class S, class... T> size_t getCommonSize(const S &a, const T &...b) {
  size_t size = a.size();
  checkSize(size, b...);
  return size;
}
template <class S> void printPieces(size_t i, ostream &s, const S &a) {
...
template <class... T> void printRow(size_t i, ostream &s, const T &...a) {
  printPieces(i, s, a...);
  s << "\n";
}

template <class... T> void printTable(const T &...a) {
...
void printMaxDiff(const std::vector<double> &a, const std::vector<double> &b) {
  ...
  std::cout << "greatest diff " << a[found] << " " << b[found] << " "
            << foundDiff << "\n";
}
void printMaxRelDiff(const std::vector<double> &a,
  ...
  std::cout << "greatest relative diff " << a[found] << " " << b[found] << " "
            << foundDiff << "\n";
}
void printGreatestWith0b(const std::vector<double> &a,
                         const std::vector<double> &b) {
  size_t size = PrintTable::getCommonSize(a, b);
  double o = 0;
  for (size_t i = 0; i < size; ++i) {
