...
public:
  SecondsCounter(double &output)
      : start(std::chrono::steady_clock::now()), output(output) {}
  ~SecondsCounter() {
...
void trial() {
  ...
  WantedMethods wm;
  wm.m_expanded = wm.m_compiled_bch = wm.m_log_of_signature = wm.m_simple_bch =
      false;
...
std::vector<int> getAllSequences(int max, int length) {
  ...
  while (1) {
    for (int i : buffer)
      *(outit++) = i;
    bool found = false;
    for (int offsetToChange = length - 1; offsetToChange >= 0;
         --offsetToChange) {
...
double numberFromCoeff(const Coefficient &coeff) {
  ...
  throw std::runtime_error(
      "a coeff which isn't just a number shouldn't be here");
}

// This function prints out the matrix from the mth level of tensor space to
// itself corresponding to the dynkin map. It is abundantly clear that this
// matrix is not symmetric and therefore the dynkin map is not an *orthogonal*
// projection (in the obvious basis).
void dynkinExperiment(const int d, const int m, bool p1, bool p2) {
  ...
  for (int i = 0; i < nWords; ++i) {
    ...
    for (int j = 1; j < m; ++j) {
      auto n = productPolynomials(
          s, &*poly, &*polynomialOfBasisElt(letters[allSeqs[offset + j]]),
          1 + j);
      poly = move(n);
      auto n2 = productPolynomials(
          s, &*polynomialOfBasisElt(letters[allSeqs[offset + m - 1 - j]]),
          &*poly2, 1 + j);
      poly2 = move(n2);
    }
    if (!p1)
...
void anagramSetCountings() {
  ...
  for (int d : {2, 3, 4, 5, 6, 7})
    for (int m : {2, 3, 4, 5, 6, 7, 8}) {
      ...
      analyseMappingMatrixLevel(mappingMatrix, m, letterOrderToBE,
                                basisEltToIndex);
      size_t total = 0;
...
bool equalLetters(const BasisElt *elt, const vector<int> &counts) {
  ...
  elt->iterateOverLetters([&](Letter l) { ++mycounts[l]; });
...
void printAMappingMatrix() {
  ...
  if (!std::is_sorted(myletters.begin(), myletters.end()) ||
      myletters.at(0) != 0)
  ...
  // vector<size_t> usedTensorIndices(usedTensorIndicesS.begin(),
  // usedTensorIndicesS.end());
...
void timeRotInv2() {
  ...
  double sum = 0;
  {
    SecondsCounter s(out);
    int level = 14;
    RotationalInvariants::Prepared(level,
                                   RotationalInvariants::InvariantType::KNOWN);
  }
  std::cout << "time taken (s): " << out << "\n";
  std::cout << sum << "\n"; // just to confound the optimizer
}
...
void tryLogBackwards() {
  ...
  if (false) {
    ...
    for (int i = 0; i < m; ++i)
      std::fill(der.m_data[i].begin(), der.m_data[i].end(),
                (CalcSignature::Number)1.0);
    // std::fill(sig.m_data[0].begin(), sig.m_data[0].end(),
    // (CalcSignature::Number)3.0);
  } else {
    WantedMethods wm;
    wm.m_expanded = wm.m_compiled_bch = wm.m_log_of_signature =
        wm.m_simple_bch = false;
    wm.m_log_of_signature = true;
    LogSigFunction lsf(LieBasis::Lyndon);
    makeLogSigFunction(d, m, lsf, wm, interrupt);
